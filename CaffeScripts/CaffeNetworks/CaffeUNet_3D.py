...
class CaffeUNet_3D:
    ...
    def add_conv(self, input, name, filter_mult):
        l = L.Convolution(input, pad=self.pad, kernel_size=self.kernel_size,
                          ...
                          weight_filler=self.weight_filler, bias_filler=self.bias_filler)
        setattr(self.net_spec, name, l)
    ...
    def create_network_structure(self):
        ...
        self.net_spec.data = L.Input(ntop=1, input_param={'shape': {'dim': [self.batch_size, self.data_channels, self.input_size, self.input_size, self.input_size]}})
        self.net_spec.target = L.Input(ntop=1, input_param={'shape': {'dim': [self.batch_size, self.label_channels, self.input_size, self.input_size, self.input_size]}}, exclude={'stage': 'deploy'})
